...
跟 Path A 一樣（`Activate Supervisor → VCF Networking with VPC`），
差別：Workload Network 的 **VPC Connectivity Profile 選 centralized 那個**。
其餘欄位（CP IP `.101`、Service CIDR `10.96.0.0/23`、Content Library 等）同 [Path A method-ui §4](../path-a-dtgw/method-ui.md)。
